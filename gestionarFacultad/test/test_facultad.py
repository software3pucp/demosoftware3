...
class TestingClasses(TestCase):
    ...
    def test_listar_facultad(self):
        print("Comenzando pruebas de: test_listar_facultad")
        c = Client()
        response = c.get('/facultades/listar/')
        if response.status_code == 200:
            print('Correcto Listar Facultad!')
        elif response.status_code == 404:
            self.assertFalse(False)


    def test_agregar_facultad(self):
        print("Comenzando pruebas de: test_agregar_facultad")
        c = Client()
    ...
    def test_listarFacultadxEsp(self):

        print("Comenzando pruebas de: listarFacultadxEsp")
        c = Client()
        id_facultad = "1"
        response = c.get('/facultades/listar/'+id_facultad+'/')
        if response.status_code == 200:
            print('Correcto Listar FacultadXEspecialidad!')
        else:
            self.assertFalse(False)


    def test_eliminarFacultad(self):
        print("Comenzando pruebas de: eliminarFacultad")
        c = Client(enforce_csrf_checks=True)
        id_facultad = "1"
        response = c.post('/facultades/eliminar/'+id_facultad+'/',follow=True)
        if response.status_code == 200:
