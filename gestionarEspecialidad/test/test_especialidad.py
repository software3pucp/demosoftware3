...
class TestingClasses(TestCase):
    ...
    def test_listarEspecialidadxCurso(self):

        print("Comenzando pruebas de: listarEspecialidadxCurso")
        c = Client()
        id_especialidad = "1"
        response = c.get('/facultades/listar/'+id_especialidad+'/')
        if response.status_code == 200:
            print('Correcto Listar EspecialidadxCurso!')
        else:
            self.assertFalse(False)



    def test_eliminarEspecialidad(self):
