from django.core.files.uploadedfile import InMemoryUploadedFile
from io import BytesIO  # StringIO and BytesIO are parts of io module in python3
from django.test import Client
import base64               # for decoding base64 image
from django.test import TestCase, override_settings

from gestionarAcreditadoras.models import Acreditadora


class TestingClasses(TestCase):

    @classmethod
    def setUpTestData(cls):
        print("Inicio de prueba del módulo ACREDITADORAS")
        print("=====================================")
        Acreditadora.objects.create(nombre="ABET", foto="img/ABET.jpg", estado=1)
        pass

    def setUp(self):
        # print("setUp: Run once for every test method to setup clean data.")
        pass

    def test_listar_acreditadoras(self):
        print("Comenzando pruebas de: test_listar_acreditadoras")
        c = Client()
        response = c.get('/gestionarAcreditadoras/listar/')
        if response.status_code == 200:
            print('Correcto Listar Acreditadoras!')
        elif response.status_code == 404:
            self.assertFalse(False)

        c = Client()
        response = c.post('/gestionarAcreditadoras/listar/', {'operacion': 'listAcred', 'estado': '1'})
        if response.status_code == 200:
            print('Correcto Listar Acreditadoras Estado 1!')
        elif response.status_code == 404:
            self.assertFalse(False)

        c = Client()
        response = c.post('/gestionarAcreditadoras/listar/', {'operacion': 'listAcred', 'estado': '0'})
        if response.status_code == 200:
            print('Correcto Listar Acreditadoras Estado 0!')
        elif response.status_code == 404:
            self.assertFalse(False)

    def test_crear_acreditadora(self):
        print("Comenzando pruebas de: test_crear_acreditadora")
        pk = '0'
        c = Client()
        response = c.get('/gestionarAcreditadoras/crear/' + pk)
        if response.status_code == 200:
            print('Correcta inicialización de Crear Acreditadora!')
        elif response.status_code == 404:
            self.assertFalse(False)

        #Insertar Nueva Acreditadora
        c = Client()
        image = InMemoryUploadedFile(
            BytesIO(base64.b64decode(TEST_IMAGE)),
            field_name='tempfile',
            name='tempfile.png',
            content_type='image/png',
            size=len(TEST_IMAGE),
            charset='utf-8',
        )
        response = c.post('/gestionarAcreditadoras/crear/' + pk, {'operacion': 'insertar', 'nombre': 'ABET', 'photo': image})
        if response.status_code == 302:
            print('Correcto Crear Acreditadora!')
        elif response.status_code == 404:
            self.assertFalse(False)

        #Editar Acreditadora sin foto
        c = Client()
        pk='1'
        response = c.post('/gestionarAcreditadoras/crear/' + pk, {'operacion': 'editar', 'nombre': 'ABET2', 'photo_name': ''})
        if response.status_code == 302:
            print('Correcto Editar Acreditadora Sin Foto!')
        elif response.status_code == 404:
            self.assertFalse(False)

        #Editar Acreditadora con foto
        c = Client()
        response = c.post('/gestionarAcreditadoras/crear/' + pk, {'operacion': 'editar', 'nombre': 'ABET2', 'photo_name': 'img', 'photo': image})
        if response.status_code == 302:
            print('Correcto Editar Acreditadora Con Foto!')
        elif response.status_code == 404:
            self.assertFalse(False)

    def test_eliminar_acreditadora(self):
        print("Comenzando pruebas de: test_eliminar_acreditadora")
        pk = '1'
        c = Client()
        response = c.get('/gestionarAcreditadoras/desactivar/'+pk)
        if response.status_code == 302:
            print('Correcta Eliminar Acreditadora!')
        elif response.status_code == 404:
            self.assertFalse(False)


TEST_IMAGE = '''

'''.strip()